...
public class DefaultLabelProvider implements KomodoObjectLabelProvider {
     ...
     */
    @Override
    public String getPath( final String displayText ) {
        ArgCheck.isNotEmpty( displayText, "displayText" ); //$NON-NLS-1$

        if ( !displayText.startsWith( ROOT_DISPLAY_PATH ) ) {
            return null;
        }

        if ( ROOT_DISPLAY_PATH.equals( displayText ) ) {
            return ROOT_PATH;
        }
        ...
        try {
            ...
            for ( final String segment : displayText.split( FORWARD_SLASH ) ) {

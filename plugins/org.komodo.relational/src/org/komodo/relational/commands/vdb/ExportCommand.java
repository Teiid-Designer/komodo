...
import static org.komodo.relational.commands.vdb.VdbCommandMessages.VDB_EXPORTED;
...
public final class ExportCommand extends VdbShellCommand {
    ...
    protected boolean doExecute() throws Exception {
        final String fileName = requiredArgument( 0, getMessage(MISSING_OUTPUT_FILE_NAME) );
        ...
        if ( file.createNewFile() || ( file.exists() && override ) ) {
            ...
            try ( final FileWriter recordingFileWriter = new FileWriter( fileName, false ) ) {
                ...
                print( MESSAGE_INDENT, VDB_EXPORTED.getMessage( vdb.getName( uow ), fileName, override ) );
                return true;
            } catch ( final Exception e ) {
                print( MESSAGE_INDENT, getMessage(ERROR_WRITING_FILE, fileName ) );
                return false;
            }
        }

        print( MESSAGE_INDENT, getMessage(OUTPUT_FILE_ERROR, fileName ) );
        return false;
    }

...
* datasource layer.
*/
public class SPParameterImpl implements SPParameter<ElementSymbolImpl> {
    ...
    private int index;
    private List<ElementSymbolImpl> resultSetColumns;      //contains List of columns if it is result set
    private List<Object> resultSetIDs;          // contains List of metadataIDs for each column in the result set
    private boolean usingDefault;
	private boolean varArg;
	private ElementSymbolImpl parameterSymbol;
     ...
     * @param expression
     */
    public SPParameterImpl(TeiidClientParser teiidParser, int index, BaseExpression expression) {
     ...
     */
    public SPParameterImpl(TeiidClientParser teiidParser, int index, int parameterType, String name) {
        this.teiidParser = teiidParser;
        setIndex(index);
    ...
    private ElementSymbolImpl createElementSymbol(String name) {
        ElementSymbolImpl symbol = teiidParser.createASTNode(ASTNodes.ELEMENT_SYMBOL);
        symbol.setName(name);
        return symbol;
    }
    ...
    public void setName(String name) {
        if (parameterSymbol == null) {
            ...
            ElementSymbolImpl es = createElementSymbol(name);
    ...
    public void addResultSetColumn(String colName, Class<?> type, Object id) {
        if(resultSetColumns == null){
            resultSetColumns = new ArrayList<ElementSymbolImpl>();
            resultSetIDs = new ArrayList<Object>();
        }

        ElementSymbolImpl rsColumn = createElementSymbol(colName);
     ...
     */
    public ElementSymbolImpl getResultSetColumn(int position){
        if(resultSetColumns == null){
    ...
    public SPParameterImpl clone() {
        SPParameterImpl clone = new SPParameterImpl(this.teiidParser, getIndex(), getExpression());
        ...
        if(this.resultSetColumns != null) {
            Iterator<ElementSymbolImpl> iter = this.resultSetColumns.iterator();
            Iterator<Object> idIter = this.resultSetIDs.iterator();
            while(iter.hasNext()) {
                ElementSymbolImpl column = iter.next();

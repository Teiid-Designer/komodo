...
public final class FunctionMethods {
    ...
    public static Long timestampDiff(String intervalType, Timestamp ts1Obj, Timestamp ts2Obj) throws TeiidClientException  {
    	return timestampDiff(intervalType, ts1Obj, ts2Obj, CALENDAR_TIMESTAMPDIFF);
    }
	
    public static Long timestampDiff(String intervalType, Timestamp ts1Obj, Timestamp ts2Obj, boolean calendarBased) throws TeiidClientException  {
        ...
        if(intervalType.equalsIgnoreCase(NonReserved.SQL_TSI_FRAC_SECOND)) {
        	if (Math.abs(tsDiff) > Integer.MAX_VALUE) {
        		throw new TeiidClientException(Messages.gs(Messages.TEIID.TEIID31144));
        	}
    	...
    	/*if (calendarBased && ((count > 0 && count > Integer.MAX_VALUE) || (count < 0 && count < Integer.MIN_VALUE))) {
    		throw new TeiidClientException(Messages.gs(Messages.TEIID.TEIID31136));
    	}*/
	...
	public static Object left(String string, Integer count)
		...
		int countValue = count.intValue();
        if(countValue < 0) {
             throw new TeiidClientException(Messages.gs(Messages.TEIID.TEIID30396, countValue));
        } 
	...
	public static String trim(String trimSpec, String trimChar, String string) throws TeiidClientException {
		if (trimChar.length() != 1) {
			 throw new TeiidClientException(Messages.gs(Messages.TEIID.TEIID30398, "TRIM CHAR", trimChar));//$NON-NLS-1$
		}
	...
	public static Object insert(String string1, Integer start, Integer length, String str2)
		...
		if(startValue < 1 || (startValue-1) > string1.length()) {
			 throw new TeiidClientException(Messages.gs(Messages.TEIID.TEIID30399, start, string1));
		} else if (len < 0) {
			 throw new TeiidClientException(Messages.gs(Messages.TEIID.TEIID30400, len));
		} else if (string1.length() == 0 && (startValue > 1 || len >0) ) {
			 throw new TeiidClientException(Messages.gs(Messages.TEIID.TEIID30401));
		}
    ...
    throws TeiidClientException {
	    ...
	    if(length < 1) {
	         throw new TeiidClientException(Messages.gs(Messages.TEIID.TEIID30402));
	    }
	    ...
	    // Get pad character
	    if(padStr.length() == 0) {
	         throw new TeiidClientException(Messages.gs(Messages.TEIID.TEIID30403));
	    }
	    // Pad string
    ...
    public static Object translate(String str, String in, String out)
        ...
        if(in.length() != out.length()) {
             throw new TeiidClientException(Messages.gs(Messages.TEIID.TEIID30404));
        }
	...
	public static Object convert(CommandContext commandContext, Object src, String type)
		...
		try {
			return dataTypeManager.transformValue(src, dataTypeManager.getDataTypeClass(type));
			 ...
			 throw new TeiidClientException(e, Messages.gs(Messages.TEIID.TEIID30405, new Object[]{src, dataTypeManager.getDataTypeName(src.getClass()), type}));
		}
	}
    ...
    public static Object context(Object context, Object expression)
        throws TeiidClientException {
         ...
         throw new TeiidClientException(Messages.gs(Messages.TEIID.TEIID30406));
    }
    ...
    public static Object rowlimit(Object expression)
        throws TeiidClientException {
         ...
         throw new TeiidClientException(Messages.gs(Messages.TEIID.TEIID30407));
    }    
    ...
    public static Object commandPayload(CommandContext context, String param) 
        ...
        // Payload was bad
         throw new TeiidClientException(Messages.gs(Messages.TEIID.TEIID30413, "commandPayload", payload.getClass().getName())); //$NON-NLS-1$
    }
	...
	public static Object array_get(Object array, int index) throws TeiidClientException, SQLException {
		try {
	...
	public static int array_length(Object array) throws TeiidClientException, SQLException {
		if (array instanceof java.sql.Array) {
	...
	public static int mvstatus(CommandContext context, String schemaName, String viewName, Boolean validity, String status, String action) throws Exception {
		if (!validity) {
			if (ErrorAction.THROW_EXCEPTION.name().equalsIgnoreCase(action)) {
				throw new TeiidClientException(Messages.gs(Messages.TEIID.TEIID31147, schemaName, viewName));
			}
			if (ErrorAction.WAIT.name().equalsIgnoreCase(action)){
				throw new TeiidClientException("BLOCKED"); //$NON-NLS-1$
			}
		}
		return 1;
	}	

...
public class NodeGenerator extends LanguageVisitor implements StringConstants {
    ...
    private class IndexKey {
        ...
        public IndexKey(LanguageObject obj, Node parent) throws RepositoryException {
            this.obj = obj;
        ...
        private NodeGenerator getOuterType() {
            return NodeGenerator.this;
        }
    }
    ...
    private Context localContext(Node parent, String reference) {
     ...
     */
    private static String toLowerCamelCase(final String value) {
        if (value == null)
            return null;
    ...
    private String resolveCurrentPath(String jcrName) throws Exception {
    ...
    private void index(LanguageObject obj, Node node) throws RepositoryException {
        if (objectIndex == null)
            objectIndex = new HashMap<>();

        IndexKey key = new IndexKey(obj, getParentNode());
        objectIndex.put(key, node);
    }

    private Node node(LanguageObject obj) throws RepositoryException {
        if (objectIndex == null)
            return null;
    ...
    private Node create(Object obj) throws Exception {
    ...
    private Node create(LanguageObject obj) throws Exception {
        Node node = node(obj);
        if (node != null)
            return node;
    ...
    private List<Value> convertToPropertyValues(Object objectValue, ValueFactory valueFactory) throws RepositoryException {
        ...
        } else if (objectValue instanceof LanguageObject) {
            result.add(valueFactory.createValue(node((LanguageObject) objectValue)));
        } else {
    ...
    private void setProperty(Node node, String name, Object value) throws Exception {
    ...
    private void setDataTypeProperty(Node node, String reference, Class<?> typeClass) throws Exception {
        DataTypeName dataTypeName = getDataTypeManager().retrieveDataTypeName(typeClass);
        setProperty(node, reference, dataTypeName.name());
    }

    private void setStatementTypeProperty(Node node, int type) throws Exception {
    ...
    private void visitObject(LanguageObject obj) {
        if (obj == null)
            return;
    ...
    private void visitObject(Node node, String reference, LanguageObject obj) {
        Context context = localContext(node, reference);
        visitObject(obj);
        context.reset();
    }

    private void visitObjects(Node node, String reference, LanguageObject[] objs) {
    ...
    private void visitObjects(Node node, String reference, Collection<? extends LanguageObject> objs) {
    ...
    public void visit(StoredProcedure obj) {
        ...
        try {
            ...
            setProperty(node, TeiidSqlLexicon.StoredProcedure.DISPLAY_NAMED_PARAMETERS_PROP_NAME, obj.displayNamedParameters());
    ...
    public void visit(SubqueryCompareCriteria obj) {
        ...
        try {
            ...
            setProperty(node, TeiidSqlLexicon.SubqueryCompareCriteria.PREDICATE_QUANTIFIER_PROP_NAME, obj.getPredicateQuantifierAsString());
    ...
    public void visit(ElementSymbol obj) {
        ...
        try {
            ...
            setProperty(node, TeiidSqlLexicon.ElementSymbol.DISPLAY_FULLY_QUALIFIED_PROP_NAME, DisplayMode.FULLY_QUALIFIED.equals(displayMode));

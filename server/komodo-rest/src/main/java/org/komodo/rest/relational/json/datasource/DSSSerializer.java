...
public class DSSSerializer extends TypeAdapter<DataSourceSchema> {
    ...
    @Override
    public void write(JsonWriter out, DataSourceSchema source) throws IOException {
        out.beginObject();

        out.name(DataSourceSchema.ID_LABEL);
        out.value(source.getId());

        out.name(DataSourceSchema.KTYPE_LABEL);
        out.value(source.getkType());

        out.name(DataSourceSchema.DESCRIPTION_LABEL);
        ...
        BUILDER.toJson(source.getProperties(), DSSPropertyListSerializer.class, out);

        out.endObject();
    }

    @Override
    public DataSourceSchema read(JsonReader in) throws IOException {

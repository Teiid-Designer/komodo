...
##
## Script for updating the version number of komodo plugins
## to a new version number (command line argument).
##
## Specifically, this updates the manifests and poms of each
## komodo plugin. It ignores the following:
##
## * Any file not in a komodo directory, eg. site/pom.xml;
...
sub process_about_properties {
  ...
  unless ($directories =~ m/\/org\.komodo\//) {
  return;
}
...
sub process {
  ...
  # Only update komodo plugins and features (and site directory)

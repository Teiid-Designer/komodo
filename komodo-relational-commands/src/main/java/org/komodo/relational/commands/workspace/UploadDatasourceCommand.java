...
public final class UploadDatasourceCommand extends WorkspaceShellCommand {
    ...
    protected CommandResult doExecute() {
        try {
            ...
            final boolean hasConnection = getWorkspaceManager().hasChild( uow,
                                                                          connection.getName(),
            ...
            if ( hasConnection && overwrite ) {
                getWorkspaceManager().removeChild( uow, connection.getName() );
            }

            // upload data source file so that it will be sequenced
            final Datasource ds = getWorkspaceManager().createDatasource( uow, null, connection.getName() );

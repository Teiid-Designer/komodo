/* Generated By:JJTree: Do not edit this line. Drop.java Version 4.3 */
/* JavaCCOptions:MULTI=true,NODE_USES_PARSER=true,VISITOR=true,TRACK_TOKENS=false,NODE_PREFIX=,NODE_EXTENDS=,NODE_FACTORY=,SUPPORT_CLASS_VISIBILITY_PUBLIC=true */
package org.teiid.query.sql.lang;

import java.util.List;

import org.komodo.spi.query.sql.lang.Drop;
import org.teiid.query.parser.TCLanguageVisitorImpl;
import org.teiid.query.parser.TeiidClientParser;
import org.teiid.query.sql.symbol.BaseExpression;
import org.teiid.query.sql.symbol.GroupSymbolImpl;

/**
 *
 */
public class DropImpl extends CommandImpl
    implements BaseTargetedCommand, Drop<BaseExpression, TCLanguageVisitorImpl> {

    /** Identifies the table to be dropped. */
    private GroupSymbolImpl table;

    /**
     * @param p
     * @param id
     */
    public DropImpl(TeiidClientParser p, int id) {
        super(p, id);
    }

    @Override
    public int getType() {
        return TYPE_DROP;
    }

    /**
     * @return the table
     */
    public GroupSymbolImpl getTable() {
        return table;
    }

    @Override
    public GroupSymbolImpl getGroup() {
        return table;
    }

    /**
     * @param table the table to set
     */
    public void setTable(GroupSymbolImpl table) {
        this.table = table;
    }

    @Override
    public List<BaseExpression> getProjectedSymbols() {
        return getUpdateCommandSymbol();
    }

    @Override
    public int hashCode() {
        final int prime = 31;
        int result = super.hashCode();
        result = prime * result + ((this.table == null) ? 0 : this.table.hashCode());
        return result;
    }

    @Override
    public boolean equals(Object obj) {
        if (this == obj) return true;
        if (!super.equals(obj)) return false;
        if (getClass() != obj.getClass()) return false;
        DropImpl other = (DropImpl)obj;
        if (this.table == null) {
            if (other.table != null) return false;
        } else if (!this.table.equals(other.table)) return false;
        return true;
    }

    /** Accept the visitor. **/
    @Override
    public void acceptVisitor(TCLanguageVisitorImpl visitor) {
        visitor.visit(this);
    }

    @Override
    public DropImpl clone() {
        DropImpl clone = new DropImpl(this.parser, this.id);

        if(getTable() != null)
            clone.setTable(getTable().clone());
        if(getSourceHint() != null)
            clone.setSourceHint(getSourceHint());
        if(getOption() != null)
            clone.setOption(getOption().clone());

        copyMetadataState(clone);

        return clone;
    }

}
/* JavaCC - OriginalChecksum=cf112d019f8dc39785f359562092f510 (do not edit this line) */
